...
<body>
	...
	<div class="container">
		...
		<c:if test="${(loginClient == null) || !(ebook.ebookState == '판매중')}">
			...
			<button type="button" disabled="disabled" class="btn btn-primary submit-search text-center">장바구니추가</button>
			</a>
		</c:if>	
	
		<c:if test="${(loginClient != null) && ebook.ebookState == '판매중'}">
			<a href="${pageContext.request.contextPath}/InsertCartController?ebookNo=${ebook.ebookNo}">
				<button type="button" class="btn btn-primary submit-search text-center">장바구니추가</button>
			</a>
		</c:if>
	</div>
